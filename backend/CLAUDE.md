...
Tests: `go test ./...` (currently only `internal/api/workflow_delete_test.go`).
...
internal/
  ...
  models/                 # GORM models: User, Project, Workflow, Execution, Worker
  ...
  api/
    ...
    auth.go / projects.go / workflows.go / executions.go / workers.go
...
- `internal/temporalexec/resolver.go` then works out, on its own goroutine, which run each event belongs to — walking Temporal's parent chain (never string-parsing ids) and caching the answer in `workflow_execution_refs`. Synthetic `workflow_(for|fork|try|catch)_*` children become a `ScopePath` on the parent's canvas; so does a child whose type matches no stored workflow — that is a switch-`then:` starting a *named workflow* declared in the same document (zigflow registers every `do:` that follows another task as a workflow named by its task key), and since its id (`<parentRunId>_N`) says nothing, its segment is `workflow_<name>` built from the type, which the frontend resolves by name; a real `run: workflow` child gets its own `Execution` row (id = its Temporal workflow id) with `ParentExecutionID`/`ParentTaskName` set, which is what powers the UI's child-run drill-down.
...
- Nothing in Temporal pushes "your workflow finished", so `watchTerminal` runs one goroutine per running execution (started by `create-execution`, by child discovery, and by a boot sweep over `status = 'running'`) to persist the terminal status, output and — new — the failure message on `Execution.Error`. It (and `reconcileStatus`) describes the workflow's *latest* run (empty run id), never `TemporalRunID`: a zigflow workflow with `metadata.canMaxHistoryLength` continues-as-new mid-run, the first run closes as `CONTINUED_AS_NEW`, and describing it left runs stuck at `running`.

**Schedules**: there is no `Schedule` model and no `/schedules` routes — a workflow's recurring trigger lives in its DSL (`schedule:` plus `document.metadata.schedule{WorkflowName,Id,Input}`), and `zigflow run` publishes it to Temporal on every worker start, deleting the schedule with the matching id first and recreating it only if the block is still there (zigflow's `pkg/zigflow/schedules.go`). So `syncWorkerRegistration` already covers create/update/cancel; `internal/api/workflow_schedule.go` only handles what zigflow structurally can't see — a **deleted** workflow (its worker stops, but the schedule it registered would keep firing at a queue nothing polls) and a **renamed** schedule id (the next worker start only cleans up the id it's about to use, orphaning the old one). Both delete the stale schedule directly, best-effort. `scheduleIDForDSL` mirrors zigflow's `metadata.GetScheduleInfo` default of `zigflow_<document.workflowType>` — keep the two in step.

**Run types, listing and deletion**: `Execution.Trigger` (column `run_trigger` — `TRIGGER` is an SQL keyword) records what started a run: `manual` (`create-execution` defaults to it) or `scheduled`; `backfill` is reserved. Nothing here *starts* a scheduled run — Temporal does, from the schedule above — so there's no row to create up front: `Resolver.adoptScheduledRun` makes one when the first CloudEvent of such a run arrives, recognising it by Temporal's `TemporalScheduledById` search attribute and matching its workflow type + task queue to a stored workflow. A parentless run without that attribute is still left unresolved rather than invented, since an unrecognised root is more likely to be someone else's workflow on the same namespace. Child runs copy their root's trigger in `Resolver.ensureChildExecution`. `GET /executions` is the cross-project list, filtered by `workflowId`/`projectId`/`trigger`/`status`/`from`/`to` and paged with `limit`/`offset`, returning `{items, total}` with workflow and project names joined in two batched reads. `DELETE /executions/{id}` and `POST /executions/delete` (bulk, all-or-nothing) live in `internal/api/execution_delete.go` and delete a run *tree* completely: child runs, events (including still-unresolved ones matched by Temporal id prefix), `workflow_execution_refs`, the resolver's in-memory cache (`Resolver.Forget`) and — best-effort, in the background — Temporal's history for the root and every child/synthetic workflow. Only root runs can be deleted; a still-running run returns 409 unless `force` is set, in which case it's terminated first. Terminal-status writes go through `persistTerminalResult` (an `UPDATE`, never `Save`) because `Save` upserts and would resurrect a run deleted while its `watchTerminal` goroutine was still waiting on Temporal.

**Deleting workflows and projects** cascades (`internal/api/workflow_delete.go`'s `deleteWorkflowCascade`): a workflow's root runs go through `deleteExecutionTrees` with `force` (in-progress runs are terminated — their worker is about to stop), then its row, then its Temporal schedule (`dropWorkflowSchedule`) and its worker process, DSL files and Workers-page row (`removeWorkerRegistration`, which keeps a row still matched by another workflow's name + task queue). `delete-project` runs that per workflow before deleting the project row, so a failure part-way never leaves workflows pointing at a missing project. Child runs a workflow contributed to *another* workflow's run tree are left with that tree. Covered by `workflow_delete_test.go` — the backend's first tests, using a temp-file SQLite DB and a Temporal address nothing listens on.
