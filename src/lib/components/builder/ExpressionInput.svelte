...
<script lang="ts">
	...
	let { value, placeholder = '', multiline = false, rows = 3, availVars, onchange }: Props = $props();
	...
	const TRANSFORMS = [
		'tostring', 'tonumber', 'ascii_downcase', 'ascii_upcase', 'length',
		'@base64', '@uri', 'keys', 'values', 'not', 'floor', 'ceil', 'round',
		'abs', 'reverse', 'unique', 'sort', 'first', 'last', 'flatten', 'type'
	];
	...
	const CAT_COLOR: Record<string, string> = {
		input: 'var(--color-primary)', env: 'var(--color-warning)', data: 'var(--color-success)',
		context: 'var(--color-info)', output: 'var(--color-secondary)'
	};
	const SRC_CAT: Record<string, string> = {
		'$input': 'input', '$env': 'env', '$data': 'data', '$context': 'context', '$output': 'output', '.': 'output'
	};
	...
	function parseSingleToken(t: string): ExprPart | null {
		...
		if (parenVar) return { kind: 'var', source: '$' + parenVar[1], field: parenVar[2], transform: parenVar[3].trim() };
		...
		if (simpleVar) return { kind: 'var', source: '$' + simpleVar[1], field: '', transform: simpleVar[2]?.trim() ?? '' };
	...
	function removeLastPart() {
		if (parts.length > 0) { parts = parts.slice(0, -1); emit(); }
	}

	function commitCurrentAsLiteral() {
		const text = currentText.trim();
		if (!text || text === '|') { currentText = ''; if (chipInputEl) chipInputEl.value = ''; return; }
	...
	function editChip(i: number) {
		...
		const txt = part.kind === 'var'
			? ((part as VarPart).field || (part as VarPart).source.replace(/^\$/, ''))
			: (part as LiteralPart).text;
		currentText = txt;
		if (chipInputEl) { chipInputEl.value = txt; chipInputEl.focus(); }
		computeChipAc(txt);
		emit();
	}
	...
	function computeChipAc(text: string) {
		if (text === '|') {
			...
			if (hasLastVar) {
				acForTransform = true;
				acItems = TRANSFORMS.map((t) => ({ label: t, insert: t, hint: '', category: 'transform' })).slice(0, 12);
				acIdx = 0;
				return;
			}
		}
	...
	function applyAcItem(item: AcItem) {
		if (item.category === 'transform') {
			...
			for (let i = parts.length - 1; i >= 0; i--) {
				if (parts[i].kind === 'var') { lastVarIdx = i; break; }
			}
			if (lastVarIdx >= 0) {
				parts = parts.map((p, i) =>
			...
			parts = [...parts, { kind: 'var', source: item.varData.source, field: item.varData.field, transform: '' }];
			currentText = '';
			if (chipInputEl) chipInputEl.value = '';
			emit();
		}
...
{:else if rawMode}
	<div class="flex items-center gap-1">
		...
		<button
			...
			onclick={toggleRaw}
			title="Switch to visual chip view"
		>chip</button>
	</div>
{:else}
	<!-- Chip-token input -->
	<div class="relative">
		...
		<div
			...
			onclick={(e) => { if (e.target === e.currentTarget) chipInputEl?.focus(); }}
			onkeydown={(e) => { if (e.key === 'Enter' && e.target === e.currentTarget) chipInputEl?.focus(); }}
		>
			{#each parts as part, i (i)}
					...
					<span class="group/chip inline-flex items-center gap-0.5 rounded bg-base-200 px-1 py-0.5">
						...
						>
							<span
								...
								style:background={srcColor(vp.source)}
							>{SRC_LETTER[vp.source] ?? '?'}</span>
							<span class="max-w-24 truncate font-mono text-[10px] text-base-content/80" title={vp.field || vp.source}>
				...
				{:else}
					...
					{#if isExprLiteral(lp.text)}
						...
						<span class="group/chip inline-flex items-center gap-0.5 rounded border border-base-300 bg-base-200/60 px-1 py-0.5">
			...
			<button
				...
				onclick={toggleRaw}
				title="Edit raw expression"
			>&lt;/&gt;</button>
		</div>

		<!-- Autocomplete dropdown -->
		{#if showAc}
			<ul class="absolute left-0 top-full z-50 mt-0.5 min-w-52 max-w-80 overflow-hidden rounded-lg border border-base-300 bg-base-100 shadow-lg">
				{#if acForTransform}
					<li class="border-b border-base-200 px-2 py-1">
						<span class="text-[9px] font-semibold uppercase tracking-wider text-base-content/40">Transform — applies to last variable</span>
					</li>
				{/if}
					...
					<li>
						...
						>
							{#if item.source}
								<span
									class="flex size-4 shrink-0 items-center justify-center rounded font-mono text-[8px] font-bold text-white"
									style:background={srcColor(item.source)}
								>{SRC_LETTER[item.source] ?? '?'}</span>
							...
							{#if item.hint}
								<span class="ml-2 shrink-0 truncate text-[10px] text-base-content/30">{item.hint}</span>
							{/if}
						</button>
					</li>
				{/each}
