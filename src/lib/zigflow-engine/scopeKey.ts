/**
 * Deterministic scope-id scheme for drill-in navigation. Every nested task body (a `for` loop's
 * `do`, a `try`'s guarded body, a `try`'s `catch.do`, one `fork` branch's body) is stored as its
 * own independent `{nodes, edges}` graph, keyed by a path built from its ancestor scope + node id.
 * Both `graph.ts` (AST <-> node-graph conversion) and the Svelte layer (drill-in buttons,
 * breadcrumbs) must use these exact same builders so a node's "nested scope" reference always
